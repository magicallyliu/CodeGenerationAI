...
import com.liuh.codegenerationbackend.model.dto.app.*;
import com.liuh.codegenerationbackend.model.entity.User;
import com.liuh.codegenerationbackend.ratelimter.annotation.RateLimit;
import com.liuh.codegenerationbackend.ratelimter.enums.RateLimitType;
...
public class AppController {
     ...
     */
    @PostMapping("/deploy")
    public BaseResponse<String> deployApp(@RequestBody AddDeployRequest appDeployRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(ObjUtil.isNull(appDeployRequest), ErrorCode.PARAMS_ERROR);
        Long appId = appDeployRequest.getAppId();
        ThrowUtils.throwIf(ObjUtil.isNull(appId) || appId <= 0, ErrorCode.PARAMS_ERROR, "应用 ID 不能为空");
        // 获取当前登录用户
        User loginUser = InnerUserService.getLoginUser(request);
        // 调用服务部署应用
    ...
    public BaseResponse<Boolean> updateApp(@RequestBody AppUpdateRequest appUpdateRequest, HttpServletRequest request) {
        if (ObjUtil.isNull(appUpdateRequest) || ObjUtil.isNull(appUpdateRequest.getId())) {
    ...
    public BaseResponse<Boolean> updateAppByAdmin(@RequestBody AppAdminUpdateRequest appAdminUpdateRequest) {
        if (ObjUtil.isNull(appAdminUpdateRequest) || ObjUtil.isNull(appAdminUpdateRequest.getId())) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
